...
1. **Validate each configured instance is a table before reading keys.**
   `Configuration#build_instance` calls `raw.key?` at
   `lib/cybort/configuration.rb:49-52` without checking `raw`. A malformed
   `[instances.foo]` value that is a scalar/array raises `NoMethodError`, which
   escapes `CLI.start` (whose rescue handles `ConfigurationError`, parser
   errors, and `SystemCallError`) instead of becoming the documented exit-2
   configuration error. Add an `is_a?(Hash)` check with an instance-specific
   `ConfigurationError`, and cover malformed root/instance shapes.
...
- Move adapter display names and item nouns out of the two hard-coded maps in
  ...
  metadata would make adding connectors less error-prone.
